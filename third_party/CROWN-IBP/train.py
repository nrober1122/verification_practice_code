...
import copy
import torch
from torch.nn import Sequential, Linear, ReLU, CrossEntropyLoss, MSELoss
import numpy as np
...
def Train_Regressor(model, t, loader, eps_scheduler, max_eps, norm, logger, verbose, train, opt, method, **kwargs):
    # if train=True, use training mode
    # if train=False, use test mode, no back prop
    ...
    #     c = torch.eye(num_class).type_as(data).unsqueeze(0) ## Originally looked like this: torch.eye(num_class).type_as(data)[labels].unsqueeze(1) - torch.eye(num_class).type_as(data).unsqueeze(0) 
    #     # remove specifications to self
    #     I = (~(labels.data.unsqueeze(1) == torch.arange(num_class).type_as(labels.data).unsqueeze(0)))
...
def main(args):
    ...
    for model, model_id, model_config in zip(models, model_names, config["models"]):
        ...
        for t in range(epochs):
            ...
            Train_Regressor(model, t, train_data, epsilon_scheduler, max_eps, norm, logger, verbose, True, opt, method, **method_param)
            if lr_decay_step:
            ...
            with torch.no_grad():
                # evaluate
                err, clean_err = Train_Regressor(model, t, test_data, EpsilonScheduler("linear", 0, 0, epoch_end_eps, epoch_end_eps, 1), max_eps, norm, logger, verbose, False, None, method, **method_param)
