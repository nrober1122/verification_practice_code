...
def unicycle_nl_datasets(num_val = 400, num_test = 200, dataset_name = "default"):
    with open(PATH + "/nfl_robustness_training/src/_static/datasets/unicycle/" + dataset_name + "/dataset.pkl", 'rb') as f:
        xs, us = pickle.load(f)
    ...
    dataset = UniDataset(torch.tensor(xs, dtype=torch.float32), torch.tensor(us, dtype=torch.float32), transform=None)
    train_set, val_set, test_set = torch.utils.data.random_split(dataset, [num_train, num_val, num_test])
    return train_set, val_set, test_set

def unicycle_nl_loaders(
        batch_size, 
        shuffle_train = True, 
        shuffle_test = False, 
    ...
    train_set, val_set, test_set = unicycle_nl_datasets(num_val, num_test, dataset_name)
